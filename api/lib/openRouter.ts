...
export function buildMessages(opts: GenerateOptions): Message[] {
  ...
  const user = [
    ...
    `  {"source": "<a real ${opts.sourceName} word>", "target": "<its ${opts.targetName} translation>", "grammar": "<part of speech>", "example": "<one short sentence in ${opts.sourceName} using the word>"}`,
    "",
    "- grammar (optional): part of speech, gender/article or conjugation note, e.g. \"noun\" or \"noun, die\".",
    `- example (optional): one short natural sentence in ${opts.sourceName}.`,
